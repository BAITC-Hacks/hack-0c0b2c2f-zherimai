...
def validate_outputs(data_dir: Path, out_dir: Path, max_seconds=300):
    ...
    require(not ((nodes.depth == 4) & (nodes.role == "terminal")).any(), "Depth-4 false terminal")
    require(not (nodes.is_seed & (nodes.role == "transit")).any(), "Seed balance used for transit")
    require((nodes.loc[nodes.in_deg + nodes.out_deg == 0, "role"] == "peripheral").all(), "Isolated node role")
    ...
    for c in clusters.itertuples(index=False):
        ...
        require(c.n_seed == int(members.is_seed.sum()), "Incorrect cluster seed count")
