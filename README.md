...
1. Аналитик помещает три исходных parquet в `data/` и выполняет команду запуска ниже.
2. Пайплайн проверяет данные, строит направленный граф, рассчитывает метрики, назначает роли и кластеры, ранжирует узлы и сохраняет CSV вместе с `out/network.html`.
3. Аналитик открывает локальный экран, выбирает кандидата из топ-50 или вводит любой gid. Карточка показывает правило роли, фактические числа, связи, состав приоритета и ограничения наблюдения.
4. По карточке и `data_requests.csv` аналитик определяет, какие сведения запросить для углублённой проверки. Результат — обоснованная очередь проверки и гипотезы, а не автоматически установленная виновность.
...
Создавать `.env` для основного запуска не требуется; приложение не загружает этот файл автоматически. Переменные `OPENAI_API_KEY` и `OPENAI_MODEL` задаются только для явно включённого опционального AI-планировщика по [отдельной инструкции](docs/ai-assistant.md).
...
Последнее объединение UI Александра и QA Ержана: **98 тестов PASS**, расчёт из новой папки без out — **4,6846 с** по внешнему таймеру; валидатор, независимый аудит и DOM-проверки прошли. Семь CSV и HTML совпали побайтно с рабочим результатом на macOS. [Актуальный протокол](docs/final-integration-verification.json).

Ержан независимо проверил **чистую установку Windows 11 AMD64 / Python 3.13.15** для `ca8b43d`: восемь локальных wheels, без pip-индекса и кэша, установка 39,942 с; первый расчёт 5,1141 с, повторный 3,2561 с по внешнему таймеру. 77 тестов вместе с пятью проверками QA-обёртки и независимый аудит прошли; 7 CSV и HTML повторились побайтно. Python socket-аудит не обнаружил обращений в проверяемых процессах; это не системный firewall. Установочный комплект хранится у Ержана отдельно и не включён в Git. [Отчёт чистой установки](docs/handoffs/offline-windows-erzhan.md).

Позже Ержан принял `c31e093` на Windows: **90 тестов**, с QA-обёрткой — **95**, все девять подмен отклонены; расчёт 3,4393 с. Повтор в кодировке cp1251 подтвердил корректную работу UTF-8. Отдельно uv загрузил новый CPython 3.13.15 и зависимости: расчёт в этой среде 6,6605 с, валидатор и аудит PASS. Подготовка uv требует сети; полный набор тестов в этой новой uv-среде не повторялся. Семь CSV и HTML повторяются побайтно в Windows; с исходными Git-файлами совпадают после нормализации LF/CRLF. [Windows-приёмка](docs/handoffs/qa-erzhan-20260923.md). Обновлённый offline-ZIP у C содержит `c31e093`, поэтому последние изменения карточки в него ещё не входят.
...
Публичная deployed-версия не размещена. Демонстрация запускается локально: после расчёта откройте `out/network.html` в браузере. Готовый HTML также включён в репозиторий; его нужно скачать, поскольку GitHub показывает исходный текст файла. Сервер и публикация сайта для проверки решения не требуются.

## Соответствие промпту организаторов для README

Инструкция участников перечисляет 11 содержательных пунктов; их расположение в этом README:

| Пункт | Где раскрыт |
|---|---|
| 1. Название проекта | Заголовок «Zherimai — Граф денег» |
| 2. Краткое описание, проблема и пользователь | Вступление перед быстрым запуском |
| 3. Что реализовано | Итоги во вступлении; [выходные файлы](#что-получает-аналитик); [интерфейс и помощник](#интерфейс-и-помощник) |
| 4. Основной пользовательский сценарий | [Как работает решение](#как-работает-решение) |
| 5. Технологии, AI-модели, API и сервисы | [Источники, технологии и вклад](#источники-технологии-и-вклад); [помощник](#интерфейс-и-помощник) |
| 6. Архитектура и взаимодействие компонентов | [Схема решения](#схема-решения) |
| 7. Установка и запуск | [Быстрый запуск](#быстрый-запуск), включая Windows, uv и offline wheels |
| 8. Повторяемый сценарий проверки | [Как проверять](#как-проверять) |
| 9. Данные и интеграции | [Данные и разведка](#данные-и-разведка); [локальный и опциональный AI](#интерфейс-и-помощник) |
| 10. Ограничения и нереализованные возможности | [Ловушки и ограничения](#ловушки-и-ограничения); [границы масштабирования](#рост-до-1-млн-узлов) |
| 11. Deployed-версия, если существует | [Развёрнутая версия](#развёрнутая-версия): публичного развёртывания нет, доступен локальный экран |
