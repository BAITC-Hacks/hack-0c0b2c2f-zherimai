...
Ержан независимо проверил расширенный `cc2c0b0` на **Windows 11 AMD64 / Python 3.13.15** с закреплёнными пакетами: два расчёта из копии без out за 3,4308/3,6406 с по внешнему таймеру; валидатор, аудит и 49 тестов прошли. Все 7 CSV и HTML повторяются побайтно в Windows; CSV также совпадают с macOS после нормализации LF/CRLF. Окружение установлено ранее: новая офлайн-установка Windows не проверялась. Александр ранее подтвердил открытие исходного экрана человеком; финальная карточка проверена программно с DOM-заглушкой. Linux и визуальная проверка финальной карточки не подтверждены. Отчёты: [QA Ержана](docs/handoffs/qa-erzhan-20260923.md), [UI Александра](docs/handoffs/ui-alexandr.md), [приёмка интеграции](docs/verification-integration.md).
