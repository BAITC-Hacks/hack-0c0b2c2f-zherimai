...
```bash
python3 -m venv .venv && .venv/bin/python -m pip install -r requirements.txt && .venv/bin/python run.py --data data --out out
```
...
Если на машине установлен Python другой версии, установите 3.13 и проверьте `python3 --version` / `py -3.13 --version`. Работа с иными версиями пакетов не считается проверенной воспроизводимостью.
...
```bash
python3 -m pip download --only-binary=:all: -r requirements.txt --dest wheelhouse
```
...
Последний проверенный прогон на текущем ноутбуке с Python 3.13.2 занял **6,19 секунды**, включая импорт библиотек, создание экрана и автоматическую проверку; это измерение конкретного прогона, не гарантия для любого оборудования. В результате 49 групп: 48 сообществ и группа изолятов 0; 9 сообществ содержат несколько seed. Роли: coordinator — 21, consolidator — 42, distributor — 43, transit — 36, terminal — 566, peripheral — 1 540. Источник — `out/report.json`; свежий запуск обновляет измерение. Дополнительно проверена установка в новом окружении без сети из локального wheelhouse: холодный запуск целиком занял 15,71 с, повторный — 3,21 с; все пять CSV совпали побайтно. Windows/Linux и визуальная проверка в реальном браузере пока не подтверждены. Подробности: [docs/verification.md](docs/verification.md).
...
`seed_reach` — количество других seed, имеющих направленный путь к узлу, без пути нулевой длины к себе. `seed_payers` — число прямых seed-плательщиков. PageRank учитывает направление и суммы (`tol=1e-10`, `max_iter=1000`, стандартный damping NetworkX). Betweenness рассчитывается **точно на направленном невзвешенном графе**, нормирована; расстояние — число шагов, сумма не трактуется как длина пути.
...
Число кластеров не подгоняется под 8 из примечания ТЗ. На сыром весе 8 означает сообщества с несколькими seed, а не все сообщества. Повторные прогоны при разных seed и статистическая устойчивость кластеризации не заявлены.
...
Устойчивость — структурный сценарий удаления узлов, а не прогноз предотвращённого ущерба. Полноценные повторяющиеся многошаговые маршруты, HITS, обученная модель, внешний LLM и подтверждённая работа на миллионе узлов не заявлены. Роль coordinator показывает структурную связность и не устанавливает организатора группы.
